...
active_digital = mongo_client['active_digital']['balances']
active_digita = mongo_client['active_digita']['balances']

for account in accounts:
  try:
    balance1 = list(active_digital.find(
      {'client': account['client'], 'venue': account['venue'], 'account': account['account']}
    ).sort("_id", -1).limit(1))[0]['balance_value']
    balance2 = list(active_digita.find(
      {'client': account['client'], 'venue': account['venue'], 'account': account['account']}
    ).sort("_id", -1).limit(1))[0]['balance_value']
  except:
    continue
