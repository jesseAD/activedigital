...
db = pymongo.MongoClient(mongo_uri, maxPoolsize=config['mongodb']['max_pool'])['active_digital_staging']
# db = MongoDB(db_name='active_digital_staging', uri=mongo_uri)

back_off = {}
exchs = {}

def public_pool(data_collectors, exchanges):
    db1 = pymongo.MongoClient(mongo_uri, maxPoolsize=config['mongodb']['max_pool'])['active_digital_staging']
...
def private_pool(data_collectors, accounts_group):
    db2 = pymongo.MongoClient(mongo_uri, maxPoolsize=config['mongodb']['max_pool'])['active_digital_staging']
    executors = [concurrent.futures.ThreadPoolExecutor(config['dask']['threadsPerPool']) for i in range(config['dask']['threadPoolsPerWorker'])]
...
def leverage_pool(leverage_collector, accounts_group):
    db3 = pymongo.MongoClient(mongo_uri, maxPoolsize=config['mongodb']['max_pool'])['active_digital_staging']
    executors = [concurrent.futures.ThreadPoolExecutor(config['dask']['threadsPerPool']) for i in range(config['dask']['threadPoolsPerWorker'])]
